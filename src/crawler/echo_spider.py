...
def get_top_sentences(text, n=5, max_words=20):
    ...
    for s in sentences[:n]:
        # Remove ASCII, smart quotes, and curly apostrophes
...
def resolve_engine(engine_name, query, config):
    ...
    elif engine_name == "gdelt":
        return search_gdelt(query)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--engine", choices=["duckduckgo", "google", "gdelt"], default="duckduckgo")
...
#     search_duckduckgo, search_google_cse
# )
# from gdelt_search import search_articles_with_gdelt  # Import the GDELT search logic
# from similarity.detector import is_match
# from dotenv import load_dotenv
...
#         return search_google_cse(query, config["GOOGLE_API_KEY"], config["GOOGLE_CSE_ID"])
#     elif engine_name == "gdelt":
#         return search_articles_with_gdelt(query)  # Use GDELT search logic
#     else:
#         raise ValueError(f"Unknown search engine: {engine_name}")
...
#     # GDELT-specific logic for fetching articles
#     if engine == "gdelt":
#         results = search_articles_with_gdelt(seed_url)  # Call the GDELT search function
#     else:
#         results = []
#         for sent in key_sentences:
#             print(f"[+] Searching with {engine}: {sent}")
#             engine_results = resolve_engine(engine, sent, config)
#             results.extend(engine_results)
#             time.sleep(delay)  # Sleep between each query
